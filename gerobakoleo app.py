...
FILE_EXCEL_REP  = "LAPORAN_HARIAN_FINAL.xlsx"
        ...
        try:
            with open(FILE_EXCEL_REP, 'rb') as f:
                requests.post(f"https://api.telegram.org/bot{TOKEN_BOT}/sendDocument", 
                              data={'chat_id': ID_OWNER, 'caption': '📊 Laporan Excel Detail'}, files={'document': f})
        except: pass
...
def rapikan_excel(filename):
    try:
        ...
        for cell in ws[1]:
            cell.fill = header_fill; cell.font = font_white
...
def simpan_laporan_lengkap(list_data):
    try:
        ...
        cols = ["TANGGAL", "JAM_LAPOR", "LOKASI", "STAFF", "JAM_MASUK", "JAM_PLG", 
                "ITEM_MENU", "HARGA", "STOK_AWAL", "SISA", "TERJUAL", "TOTAL_OMZET", 
        ...
        df_final.to_excel(FILE_EXCEL_REP, index=False); rapikan_excel(FILE_EXCEL_REP)
        return True
    except: return False

# ================= MAIN APP =================
def main():
    st.set_page_config(page_title="Sistem Gerobak Final", page_icon="🥤")
    
    if TOKEN_BOT == "PASTE_TOKEN_BOT_ANDA_DISINI":
        st.error("⚠️ TOKEN KOSONG! Baris 10."); st.stop()
    ...
    if st.session_state['user']:
        ...
        if user == "OWNER":
            ...
            t_adm1, t_adm2, t_adm3 = st.tabs(["Lokasi", "Menu", "Staff"])
            with t_adm1:
            ...
            with t_adm2:
                ...
                m_b = st.text_input("Menu"); h_b = st.number_input("Harga", step=500)
                if st.button("Simpan Menu"): MENU[m_b]=int(h_b); save_json(FILE_DB_MENU, MENU); st.rerun()
            with t_adm3: st.write(STAFF)

        # OPERASIONAL
        lokasi = st.selectbox("Lokasi Gerobak:", list(LOKASI.values()))
        shift = GEROBAK.get(lokasi)

        tb1, tb2 = st.tabs(["☀️ BUKA TOKO", "🌙 TUTUP TOKO"])
            ...
            if shift and shift['pin_pic'] != pin: st.error("Shift orang lain")
                ...
                with st.form("buka"):
                    ...
                    if st.form_submit_button("SIMPAN"):
                        ...
                        save_json(FILE_DB_GEROBAK, GEROBAK); kirim_telegram(f"☀️ *OPENING* {lokasi}\n👤 {user}\n🕒 {jm}"); st.rerun()

        with tb2:
            if not shift: st.info("Buka toko dulu")
            elif shift['pin_pic'] != pin: st.error("Bukan shift Anda")
            else:
                with st.form("tutup"):
                    st.write("📊 **Hitung Sisa Barang & Penjualan:**")
                    list_excel = []; total_omzet = 0; tgl = datetime.now().strftime("%d-%m-%Y"); jam_plg = datetime.now().strftime("%H:%M")
                    
                    txt_terjual = [] # Rincian laku
                    txt_stok_sisa = [] # Rincian sisa
                    
                    for item, harga in MENU.items():
                        awal = int(shift['stok'].get(item, 0))
                        sisa = st.number_input(f"Sisa {item} (Awal: {awal})", min_value=0, max_value=awal)
                        laku = awal - sisa; duit = laku * harga; total_omzet += duit
                        
                        list_excel.append({
                            "TANGGAL": tgl, "JAM_LAPOR": jam_plg, "LOKASI": lokasi, "STAFF": user,
                            "JAM_MASUK": shift['jam_masuk'], "JAM_PLG": jam_plg,
                            "ITEM_MENU": item, "HARGA": harga, "STOK_AWAL": awal, "SISA": sisa,
                            "TERJUAL": laku, "TOTAL_OMZET": duit
                        })
                        
                        if laku > 0: txt_terjual.append(f"• {item}: {laku}")
                        txt_stok_sisa.append(f"• {item}: {sisa}")

                    st.markdown(f"### 💰 Total: {format_rupiah(total_omzet)}")
                    tn = st.number_input("Tunai", step=500); qr = st.number_input("QRIS", step=500); nt = st.text_area("Catatan")
                    
                    if st.form_submit_button("KIRIM LAPORAN"):
                        list_excel.append({"TANGGAL": tgl, "JAM_LAPOR": jam_plg, "LOKASI": lokasi, "STAFF": user, "ITEM_MENU": "TOTAL SETORAN", "TOTAL_OMZET": total_omzet, "SETOR_TUNAI": tn, "SETOR_QRIS": qr, "CATATAN": nt})
                        simpan_laporan_lengkap(list_excel); kirim_file_excel_telegram()
                        
                        # --- PESAN TELEGRAM LENGKAP ---
                        selisih = (tn+qr) - total_omzet
                        stat = "PAS ✅" if selisih==0 else (f"MINUS {format_rupiah(abs(selisih))} ⚠️" if selisih < 0 else f"LEBIH {format_rupiah(selisih)} ℹ️")
                        
                        msg = (f"🌙 *CLOSING LAPORAN*\n"
                               f"📍 *{lokasi}*\n👤 {user}\n"
                               f"🕒 {shift['jam_masuk']} - {jam_plg}\n\n"
                               f"📊 *RINCIAN JUALAN:*\n{chr(10).join(txt_terjual) if txt_terjual else '- Tidak ada jualan'}\n\n"
                               f"📦 *SISA STOK GEROBAK:*\n{chr(10).join(txt_stok_sisa)}\n\n"
                               f"💰 *Omzet: {format_rupiah(total_omzet)}*\n"
                               f"💵 Tunai: {format_rupiah(tn)}\n"
                               f"💳 QRIS: {format_rupiah(qr)}\n"
                               f"📝 Status: *{stat}*\n"
                               f"Catatan: {nt if nt else '-'}")
                        
                        kirim_telegram(msg)
                        del GEROBAK[lokasi]; save_json(FILE_DB_GEROBAK, GEROBAK); st.balloons(); st.rerun()
    else: st.info("Login dulu")
